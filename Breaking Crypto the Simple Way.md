🔗 [Link to the Room](https://tryhackme.com/room/breakingcryptothesimpleway)

## 🏷️Exploiting common cryptographic mistakes.

1. [Brute-forcing Keys](#bruteforcing-keys)<br>
  1.1 [Characteristic of Strong Keys](#characteristic-of-strong-keys)<br>
  1.2 [Math of RSA](#math-of-rsa)<br>
  1.3 [How Factorisation Time Increases Exponentially](#how-factorisation-time-increases-exponentially)<br>
  1.4 [What is "P's and Q's"?](#what-is-ps-and-qs)<br>
  1.5 [Exercise](#exercise)<br>
2. [Breaking Hashes](#breaking-hashes)<br>
  2.1 [Common Vulnerabilities in Hashing](#common-vulnerabilities-in-hashing)<br>
  2.2 [SHA-256 Isn't ideal for Password Hashing](#sha256-isnt-ideal-for-password-hashing)<br>
  2.3 [Choosing the Right Hashing Function](#choosing-the-right-hashing-function)<br>
  2.4 [Challenge](#challenge)<br>
  2.5 [Solution](#solution)<br>
3. [Exposed Keys](#exposed-keys)<br>
  3.1 [Risks of Exposing Cryptographic Keys in Client-Side Code](#risks-of-exposing-cryptographic-keys-in-clientside-code)<br>
  3.2 [Common Scenarios of Key Exposure](#common-scenarios-of-key-exposure)<br>
  3.3 [Exercise](#exercise)<br>
  3.4 [Key Takeaways](#key-takeaways)<br>
4. [Bit Flipping Attacks](#bit-flipping-attacks)<br>
  4.1 [What is Unauthenticated Encryption?](#what-is-unauthenticated-encryption)<br>
  4.2 [Bit Flipping Attacks](#bit-flipping-attacks)<br>
  4.3 [Exercise](#exercise)<br>



&nbsp;

# 📚 Study Notes #

<!-- INTRODUCTION HERE -->

&nbsp;

# Brute-forcing Keys

&nbsp;





&nbsp;


## Characteristic of Strong Keys


&nbsp;

## Math of RSA



&nbsp;

## How Factorisation Time Increases Exponentially


&nbsp;

## What is "P's and Q's"?


&nbsp;

## Exercise





&nbsp;

---
><details><summary>❓What is the flag?</summary></details>
---

&nbsp;

# Breaking Hashes

&nbsp;




&nbsp;

## Common Vulnerabilities in Hashing





&nbsp;

## SHA-256 Isn't ideal for Password Hashing





&nbsp;

## Choosing the Right Hashing Function



&nbsp;

## Challenge


&nbsp;

## Solution




&nbsp;

---
><details><summary>❓What is the secret used to encrypt the message?</summary>sunshine</details>
---
&nbsp;

# Exposed Keys

&nbsp;




&nbsp;

## Risks of Exposing Cryptographic Keys in Client-Side Code



&nbsp;

## Common Scenarios of Key Exposure




&nbsp;

## Exercise



&nbsp;

## Key Takeaways


&nbsp;

---
><details><summary>❓What is the flag?</summary></details>
---

&nbsp;

# Bit Flipping Attacks

&nbsp;





&nbsp;

## What is Unauthenticated Encryption?



&nbsp;

## Bit Flipping Attacks




&nbsp;

## Exercise






&nbsp;

---
><details><summary>❓What is the flag?</summary></details>
---


&nbsp;

<!-- ADD CONCLUSION / SUMMARY HERE-->



&nbsp;
